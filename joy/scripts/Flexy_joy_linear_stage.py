...
### TO DO 2022/07/27: build the Flexy_linear_stage.io
...
def callback(data):
    ...
    stepper_speed.data = remap(data.axes[2], old_max=1.0, old_min=-1.0, new_max=0.0, new_min=11999.0)
    if stepper_speed.data == 0.0:
        stepper_speed.data = remap(data.axes[5], old_max=1.0, old_min=-1.0, new_max=0.0, new_min=-11999.0)

def listener():
    global stepper_speed
    rospy.init_node('sub_joy_pub_linearStage', anonymous=True)
    r = rospy.Rate(240) # 10hz
    rospy.Subscriber("/joy", Joy, callback)
    pub_stepper = rospy.Publisher('/desired_stepper_speed', Float64, queue_size=1)
