...
def wave():
    ...
    rospy.Subscriber("/joint_states_from_arduino_2", Floats, callback_angle_motor_1, queue_size=1)
	...
	if init is True:
		...
		init = False
			...
			if not(q_1 < 0.3*rad_to_deg):  # according to the amplitude we want to reach we can change the number 0.3 with the radians we desire
				flag = 1
				sample +=1			
			amplitude.data = a
	        	rospy.loginfo(amplitude)
			...
			if not(q_1 > -0.3*rad_to_deg):   # according to the amplitude we want to reach we can change the number 0.3 with the radians we desire (do not touch the minus)
				flag = 0
				sample +=1
			amplitude.data = -a
			rospy.loginfo(amplitude)
