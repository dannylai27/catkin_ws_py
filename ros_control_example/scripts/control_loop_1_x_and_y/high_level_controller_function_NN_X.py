...
def controller(pos_in_x, pos_in_y, cam_x, cam_y, error_x, error_y, q_1_old, q_2_old, new_model, data_buffer_x_NN, data_buffer_y_NN, na, nb, L):
    ...
    p_controller_x = 0.5 #0.8 # old p 0.5
...
#    cam_x = ( data_buffer_x_NN[0][0] + data_buffer_x_NN[1][0] + data_buffer_x_NN[2][0] ) / 3.0
...
#    print("-----inzio ciclo------")
    ...
    forecast_data_array[0] = data_buffer_x_NN[0][4]
    ...
    for z in range(L):
      if(z==0):
          for r in range(na+nb+2):
            temp_array[0][r] = data_buffer_x_NN[0][r]
...
# a_predicted = ( sum_y_predicted * sum_x2 - sum_x * sum_xy_predicted ) / ( n * sum_x2 - sum_x * sum_x )
    ...
    if cam_x != 1000:															# if we detect a circle
        if (error_x * error_x > 50):													# if the error is greater than the tollerance
            if ((q_1_old - data_buffer_x_NN[3][na+nb])*(q_1_old - data_buffer_x_NN[3][na+nb]) > 50):					# if the robot is moving to correct the error 
    ...
    print(data_buffer_x_NN[0][0])
    print(data_buffer_x_NN[1][0])
    print(data_buffer_x_NN[2][0])
    print(data_buffer_x_NN[3][0])
