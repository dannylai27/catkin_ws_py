...
def callback(camera_pos, desired_pos, joint_1, joint_2):
...
def error_computation():
    ...
    j1_sub = message_filters.Subscriber('/joint_states_from_arduino_1', Floats)
    j2_sub = message_filters.Subscriber('/joint_states_from_arduino_2', Floats)

    ts = message_filters.ApproximateTimeSynchronizer([dp_sub, dc_sub, j1_sub, j2_sub], 1, 0.15, allow_headerless=True) # 0.15 è lo slope, massimo delta t tra i tempi di arrivo (in secondi) consentito per accoppiare 2 messaggi, setroppo stretto ed i messaggi dai 2 topic arrivano sempre ad un delta t maggiore di quello indicato non verrà mai sottoscritto nulla ed i valori pubblicati saranno sempre quelli di inizializzazione 0 0 0 0 0 0
